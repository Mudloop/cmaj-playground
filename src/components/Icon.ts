...
export class IconRegistry {
	...
	static register = (type: string, icon: string | TemplateResult) => {
		if (typeof icon === 'string' && icon.trim().startsWith('<')) this.icons.set(type, html`${unsafeHTML(icon)}`);
		else this.icons.set(type, icon);
	}
	static getIcon = (key?: string): string | TemplateResult | undefined => this.icons.get(key ?? '')
	static {
		for (let [key, icon] of Object.entries(icons)) {
			this.register(key, icon as string);
		}
...
@customElement('ui-file-icon') export class FileIcon extends Icon {
	...
	getIcon = () => {
		...
		const lang = MonacoEditor.getLanguage(this.path!);
